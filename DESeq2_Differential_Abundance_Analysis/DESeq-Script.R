...
#resultsNames(DESEQ_merged)  # check coefficient names first
...
us_ecigarettes_vs_us_control_vol_plot <- us_ecigarettes_vs_us_control |> 
  mutate(significant = padj<0.05 & abs(log2FoldChange)>2)  |>  # new significant column created with set thresholds 
  ggplot() + 
  geom_point(aes(x = log2FoldChange, y = -log10(padj),  col=significant)) + # using the -log10() function to transform padj values
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") + # set boundary lines on x- and y-axes to plot data into quadrants
...
sigASVs <- us_ecigarettes_vs_us_control  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>   # changed to more stringent threshold (i.e., from padj < 0.05 to <0.01) 
  dplyr::rename(ASV=row) # rename column "row" to be called ASV
# Get only asv names into a vector 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
# Prune phyloseq file
  # Create a new object. Extract taxa table from phyloseq object and filter for only the significant ASV (padj <0.01) 
merged_DESeq <- prune_taxa(sigASVs_vec,filtered_phyloseq)
sigASVs <- tax_table(merged_DESeq) |>  as.data.frame() |> 
  rownames_to_column(var="ASV")  |> 
  right_join(sigASVs)  |> 
  arrange(log2FoldChange) |> # in Ascending order 
  mutate(Genus = make.unique(Genus)) |>  #only list unique genus to avoid repeating Genera
  mutate(Genus = factor(Genus, levels=unique(Genus)))
# Generate Horizontal Bar Plot
us_ecigarettes_vs_us_control_bar_plot <- ggplot(sigASVs) +
        geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
        geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- us_tobacco_vs_us_control  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>   # changed to more stringent threshold (i.e., from padj < 0.05 to <0.01) 
...
# Generate Horizontal Bar Plot
us_tobacco_vs_us_control_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
...
sigASVs <- us_tobacco_vs_us_ecigarettes  |>  
...
# Generate Horizontal Bar Plot
us_tobacco_vs_us_ecigarettes_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- cn_ecigarettes_vs_cn_control  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>  
  dplyr::rename(ASV=row) 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
...
# Generate Horizontal Bar Plot
cn_ecigarettes_vs_cn_control_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- cn_tobacco_vs_cn_control  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>  
  dplyr::rename(ASV=row) 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
...
# Generate Horizontal Bar Plot
cn_tobacco_vs_cn_control_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- cn_tobacco_vs_cn_ecigarettes  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>  
  dplyr::rename(ASV=row) 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
...
# Generate Horizontal Bar Plot
cn_tobacco_vs_cn_ecigarettes_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- cn_control_vs_us_control  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>  
  dplyr::rename(ASV=row) 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
...
# Generate Horizontal Bar Plot
cn_control_vs_us_control_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- cn_ecigarettes_vs_us_ecigarettes  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>  
  dplyr::rename(ASV=row) 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
...
# Generate Horizontal Bar Plot
cn_ecigarettes_vs_us_ecigarettes_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
sigASVs <- cn_tobacco_vs_us_tobacco  |>  
  filter(padj<0.01 & abs(log2FoldChange)>2) |>  
  dplyr::rename(ASV=row) 
sigASVs_vec <- sigASVs |> 
  pull(ASV)
...
# Generate Horizontal Bar Plot
cn_tobacco_vs_us_tobacco_bar_plot <- ggplot(sigASVs) +
  geom_bar(aes(x=Genus, y=log2FoldChange), stat="identity")+ 
  geom_errorbar(aes(x=Genus, ymin=log2FoldChange-lfcSE, ymax=log2FoldChange+lfcSE)) + # error bar included 
...
p_us <- sigASVs_all |> 
  ...
  ggplot(aes(x = Genus, y = log2FoldChange)) +
