...
merged_phyloseq <- merge_phyloseq(mpt, cn_phyloseq_obj)
# if with phylo_tree:
# ERROR: Error in FUN(X[[i]], ...) : one tree has a different number of tips

## Save merged phylosq object (Added by Kayla)
# Also changed ps_merged to merged_phyloseq
save(merged_phyloseq, file="merged_phyloseq.RData")

#### Integrity checks #### 
merged_phyloseq #merged WITHOUT phylo trees

# Expect these:
nsamples(merged_phyloseq)                  # total samples = n_mpt + n_cn (90+33=123)
