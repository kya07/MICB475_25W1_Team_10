##Functional analysis comparing tobacco vs control in chinese samples -->  
...
ggsave("cn_tob_v_cont_results/Plots/KEGG Error Bar Fixed.png",
...
sig_features <- tob_v_cont_daa_annotated_results_df |>
  filter(p_adjust < 0.0000001, abs(log2FoldChange) > 5) |>
  pull(feature) |> unique()

ko_relab = read.delim('china/KO_metagenome_out/pred_metagenome_unstrat.tsv',row.names = 1) |> 
...
pheatmap(stats, 
         ...
         fontsize_row = 10, 
         fontsize_col = 10,
         filename = "cn_tob_v_cont_results/Plots/KEGG Heatmap.png",
         height= 9, width = 9)
...
ggsave("cn_tob_v_cont_results/Plots/Pathway_PCA.png", plot = p, width = 8, height = 6)
