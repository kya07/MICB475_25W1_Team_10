##Functional analysis comparing tobacco vs control in us samples -->  
...
#Filter only to compare us control vs tobacco 

tob_v_cont_meta <- meta |>
  filter(Combined %in% c("us_tobacco", "us_control"))

tob_v_cont_ko <- ko |>
  select (all_of(tob_v_cont_meta$sample_name))

####Run Differential abundance analysis (DAA)
tob_v_cont_DAA <- pathway_daa(abundance = tob_v_cont_ko,
                              metadata = tob_v_cont_meta,
                              group = "Combined",
                              daa_method = "LinDA",
...
saveRDS(tob_v_cont_daa_annotated_results_df, 'us_tob_v_cont_results/tob_v_cont_daa_annotated_results_df.rds')

tob_v_cont_daa_annotated_results_df = readRDS('us_tob_v_cont_results/tob_v_cont_daa_annotated_results_df.rds')
...
ggsave("us_tob_v_cont_results/Plots/KEGG Error Bar Fixed.png",
       plot = tob_v_cont_peb, width = 20, height = 10)

###Making Heat Map 
# pulling out the significant features that we worked with oreviously 
sig_features <- tob_v_cont_daa_annotated_results_df |>
...
pheatmap(stats, 
         ...
         fontsize_row = 10, 
         fontsize_col = 10,
         filename = "us_tob_v_cont_results/Plots/KEGG Heatmap.png",
         height= 9, width = 9)

# Generate pathway PCA plot
p <- pathway_pca(abundance = tob_v_cont_ko,
                 metadata = tob_v_cont_meta, 
                 group = "Combined")

ggsave("us_tob_v_cont_results/Plots/Pathway_PCA.png", plot = p, width = 8, height = 6)
