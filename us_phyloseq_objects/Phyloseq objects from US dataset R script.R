...
metaFP <- file.path("us_metadata_new.tsv")
meta <- read_delim(file=metaFP, delim = "\t")

otuFP <- file.path("us_exports", "table_export", "feature-table.txt")
otu <- read_delim(file=otuFP, delim = "\t", skip=1)

taxfp <- file.path("us_exports", "taxonomy_export", "taxonomy.tsv")
tax <- read_delim(taxfp, delim="\t")

phylotreefp <- file.path("us_exports","rooted-tree_export", "tree.nwk")
